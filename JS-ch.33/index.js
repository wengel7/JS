...
//!setInterval() = just like setTimer invokes a function but repeatedly 
//!                asynchronous function(don't pause execution)
...
// promise0.then(() => {console.log("Thank you for waiting")});
//?Last but not least we can use promise in such a way (we can use this in less syntax with async)
...
//!async = make a function return a promise instade a function being a function itself
//?File load is a coming chapter but let use it as in a example as we have used it in promise before 
...
//!await = make an async function wait for Promise
...
//     }
//   }
  //?instade of writing it... 
//?   loadFile().then(value => console.log(value))
//?   .catch(error => console.log(error));
//?this way we can use use wait key word and by putting it in an async function

// async function startProcess(){
//    let isLoad = await loadFile();
//   console.log(isLoad); 
// }
//   startProcess();
//?The function above does not catch errors which is a problem so we need to remodify it with "try and catch"

// async function startProcess(){
//    try{ let isLoad = await loadFile();
//    console.log(isLoad); }
//    catch(error){
//     console.log(error);
//    }
//  }
//    startProcess();
 //?This is helpful when we have multiple async functions next time
